...
function dragStart(e) {
  startPositionId = e.target.parentNode.id;
...
function dragDrop(e) {
  ...
  const correctGo = draggedElement.firstChild.classList.contains(palyerGo);
  const taken = e.target.classList.contains("piece");
  const valid = checkIfValidMove(e.target);
  const opoonentGo = palyerGo === "white" ? "black" : "white";
  const takenByOpponent = e.target.firstChild?.classList.contains(opoonentGo);
  ...
  switch (piece) {
    ...
    case "pawn":
      ...
      if (
        ...
        startId + width + 1 === targetId ||
        (startId + width - 1 === targetId &&
          dowument.querySelector(`[square-id="${startId + width - 1}"]`)
            .firstchild) ||
        document.querySelector(`[square-id="${startId + width + 1}"]`)
          .firstchild
      ) {
...
function checkForWin() {
  ...
  if (
    !Kings.isSameNode((king) => king.firstChild.classList.contains("white"))
  ) {
  ...
  if (
    !Kings.isSameNode((king) => king.firstChild.classList.contains("black"))
  ) {
