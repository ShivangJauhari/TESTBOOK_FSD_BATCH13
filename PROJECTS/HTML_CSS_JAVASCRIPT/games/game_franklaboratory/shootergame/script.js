    ...
    update(deltatime){
        ...
        if(this.timeSinceFlap > this.flapInterval){           
            ...
            if(this.hasTrail)
                {
                    for(let i = 0; i < 5; i++){
                        particles.push(new Particle(this.x, this.y + this.height*0.5, 2, this.color));
                    }
                }        
...
class Particle{
    constructor(x, y, size, color){
        ...
        this.y = y + this.size*0.1+ Math.random()*50 - 25;
        this.radius = Math.random() * this.size*0.50;
...
function drawGameOver(){
    ...
    ctx.fillStyle = 'white';
    ctx.fillText('GAME OVER, your score is: ' + score, canvas.width*0.5, canvas.height*0.5+5);
}
...
function animate(timestamp){
    ...
    if(timeToNextRaven > ravenInterval){
        ...
        // ravenInterval = 5000 * Math.random();
    }
